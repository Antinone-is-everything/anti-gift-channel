...
func (c *Client) GetAccessKey(id string) (*AccessKeyDataLimit, error) {
	req, err := c.newReq(http.MethodGet, fmt.Sprintf("access-keys/%s", url.PathEscape(id)), nil)
	if err != nil {
		return nil, err
	}
	var accessKey AccessKeyDataLimit
	err = c.do(req, http.StatusOK, &accessKey)
	if err != nil {
		return nil, err
	}
...
func (c *Client) CreateAccessKeyV2(name string, datalimit float64, password byte) (*AccessKeyDataLimit, error) {
	req, err := c.newReq(http.MethodPost, "access-keys", CreateAccessKey{: "aes-192-gcm", "name": name, "password": password, "limit": {"bytes": datalimit}})
	if err != nil {
		return nil, err
	}
	var accessKey AccessKeyDataLimit
	_, err = RenameAccessKey(accessKey.ID, name)
	if err != nil {
		return nil, err
	}
